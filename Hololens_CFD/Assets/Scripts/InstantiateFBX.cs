...
{
    string assetName = "Shape_IndexedFaceSet.001reference_velocity.py_timestep1_time50.0_";
    ...
    IEnumerator SaveAndDownload(string url, string asstName)
    {
        ...
        GameObject fbxObject = AssetDatabase.LoadAssetAtPath<GameObject>(saveTo);
        if (fbxObject != null)
        {
            // Instantiate the FBX at the specified position
            Instantiate(fbxObject, spawnPosition, Quaternion.identity);
        }
        else
        {
            Debug.LogError("Failed to load FBX from file: " + saveTo);
        }
    }
